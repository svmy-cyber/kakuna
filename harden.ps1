...
function Write-Verify {
    ...
    $icon = if ($IsEnabled) { "✓" } else { "✗" }
    $color = if ($IsEnabled) { "Green" } else { "Red" }
    Write-Host "$icon $($Message): $($Status)" -ForegroundColor $color
}
...
$systemConfigTasks = @(
    ...
    @{
        ...
        Action = {
            $taskName = "HourlyFullVirusScanAt10Past"
            if (Get-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue) {
                Unregister-ScheduledTask -TaskName $taskName -Confirm:$false | Out-Null
            }
            ...
            $scriptPath = "$env:SystemRoot\Temp\HourlyFullVirusScanAt10Past.ps1"
            Set-Content -Path $scriptPath -Value $scriptContent
            
            $action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-File `"$scriptPath`""
            $trigger = New-ScheduledTaskTrigger -Once -At "00:10AM" -RepetitionInterval (New-TimeSpan -Hours 1)
        ...
        Verify = {
            $taskName = "HourlyFullVirusScanAt10Past"
            if (Get-ScheduledTask -TaskName $taskName -ErrorAction SilentlyContinue) {
                Write-Verify "Hourly full virus scan" "Enabled" $true
            } else {
                Write-Verify "Hourly full virus scan" "Not Enabled" $false
            }
        }
    },
    @{
